...
def load_default_ipfw_config():
    """
    Returns the default IpfwConfig
    """
    return IpfwConfig(
...
def load_toml_ipfw_config(filename):
    """Returns a IpfwConfig created by loading a TOML file from the
    filesystem.
    ...
    Returns:
        config (IpfwConfig): The IpfwConfig created from the stored
            toml file.
...
def merge_ipfw_config(configs):
    """
    Given a list of IpfwConfig objects, merges them into a single
    IpfwConfig, giving priority in the order of the configs
    (first has highest priority).

    Args:
        config (list of IpfwConfigs): The list of ipfw configs that
            must be merged together

    Returns:
        config (IpfwConfig): One IpfwConfig that combines all of the
            passed in configs.
    """
    connect = None
